                  ...
                  return (
                    <div key={movie.id} className="group relative block animate-fade-in">
                      <Link href={`/movies?id=${movie.id}`} className="cursor-pointer block">
                        ...
                        <span className="block text-body-md font-bold truncate group-hover:text-primary transition-colors">{movie.title || movie.name}</span>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-stack-lg">
              {loadingMood ? (
                ...
                moodMovies.map((movie: any, idx: number) => {
                  ...
                  return (
                    <div key={movie.id} className="relative group rounded-xl overflow-hidden">
                      <Link href={`/movies?id=${movie.id}`} className="glass relative overflow-hidden group cursor-pointer transition-all hover:scale-[1.02] block border border-white/10 w-full h-full">
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full bg-black/60 hover:bg-black/80 backdrop-blur-md flex items-center justify-center text-white border border-white/10 transition-all duration-200 cursor-pointer shadow-lg hover:scale-110 active:scale-95 group-hover:opacity-100 md:opacity-0"
                      >
                        {isLoading ? (
                          <div className="w-3 h-3 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                        ) : (
